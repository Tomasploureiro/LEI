...
class GZIP:
	...
	def decompressData(self, arvoreHLIT, arvoreHDIST):
		...
		while(pos != 256):
			bit = self.readBits(1)
			pos = arvoreHLIT.nextNode(str(bit))
        
			if(pos >= 0):  
				if(pos <= 255):
					outputArray.append(pos)
					...
					if (257 <= pos <= 264):
						...
						for simbolo in dicHLIT:
							if(pos == int(simbolo)):
							    comp = dicHLIT[simbolo][0]
							    bitsExtra = self.readBits(dicHLIT[simbolo][1])
							    comp += bitsExtra
							    array[0] = comp
                    
					codDist = -2
					while(codDist < 0):
						codDist = arvoreHDIST.nextNode(str(self.readBits(1)))
                            
					arvoreHDIST.resetCurNode()
					...
					if(codDist < 4):
						...
						for simbolo in dicHDIST:
							if(codDist == int(simbolo)):
							    dist = dicHDIST[simbolo][0]
							    bitsExtra = self.readBits(dicHDIST[simbolo][1])
							    dist += bitsExtra
							    array[1] = dist
                                
					recuo = len(outputArray) - array[1]
					for i in range (array[0]):
						outputArray.append(outputArray[recuo + i])

				arvoreHLIT.resetCurNode()
